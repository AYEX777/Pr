...
const seedSensorHistory = async () => {
  ...
  try {
    ...
    // Créer la table avec UUID comme spécifié
    await pool.query(`
    ...
    // Créer les index
    await pool.query('CREATE INDEX idx_sensor_readings_sensor_id ON sensor_readings(sensor_id)');
    await pool.query('CREATE INDEX idx_sensor_readings_created_at ON sensor_readings(created_at)');

    console.log('✅ Table sensor_readings créée/vérifiée');
    ...
    for (const sensor of sensorsResult.rows) {
      ...
      // Pas besoin de vérifier, on génère toujours les données
